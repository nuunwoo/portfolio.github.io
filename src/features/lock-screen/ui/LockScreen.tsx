...
} from './animations/lockScreenAnimations';
    ...
    <LockScreenMotion
      ...
      <div className={styles.overlay}>
        <div className={styles.centerWrap}>
          <motion.div
            animate={getLockScreenClockMotion(isTransitioningOut)}
            transition={getLockScreenClockTransition(disableTransition)}
            initial={false}>
            <p className={styles.dateText}>{formatLockScreenDate(currentDate)}</p>
            <h1 className={styles.timeText}>{formatLockScreenTime(currentDate)}</h1>
          </motion.div>
          <motion.div
            className={styles.unlockWrap}
            animate={getLockScreenProfileMotion(isTransitioningOut)}
            transition={getLockScreenProfileTransition(disableTransition)}
            initial={false}>
            <div className={styles.avatar}>
              <span>👤</span>
            </div>
            <p className={styles.nameText}>이현우</p>
            <p className={styles.hintText}>클릭하거나 Enter 키를 눌러 잠금 해제</p>
          </motion.div>
        </div>
      </div>
    </LockScreenMotion>
