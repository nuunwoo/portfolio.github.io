...
const MenuBarLeftMenu = ({ items, activeKey, isMenuOpen, onItemSelect, onAppleClick }: MenuBarLeftMenuProps) => {
  const handleAppleClick = useCallback((event: ReactMouseEvent<HTMLButtonElement>) => {
    onItemSelect(APPLE_MENU_KEY, event.currentTarget);
    onAppleClick();
  }, [onAppleClick, onItemSelect]);

  const handleAppleMouseEnter = useCallback((event: ReactMouseEvent<HTMLButtonElement>) => {
